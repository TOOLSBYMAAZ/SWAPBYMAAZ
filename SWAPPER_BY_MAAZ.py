...
# Bot configuration
BOT_TOKEN = "8055559366:AAFSj7mFPbBz4k8vEWJyD-TSPu-2ePdxsRQ"  # Replace with your token
DEFAULT_WEBHOOK_URL = "https://discord.com/api/webhooks/13588761041/-5MqmNYtm71bGnyH7Q5uxOcX"  # Replace
TELEGRAM_CHANNEL_ID = "-1002690912519"  # Replace with your channel ID
PREMIUM_USERS = [1883932836]  # Replace with your Telegram user ID
...
def send_discord_webhook(webhook_url, username, action, footer=None):
    ...
    title = "KORN Swapper" if action == "Swapped" else "KORN Swapped"
    ...
    embed = {
        ...
        "footer": {"text": f"By : {footer or 'KORN V4'}"}
    }
...
def send_notifications(chat_id, username, action):
    footer = session_data[chat_id]["name"] or "KORN V4"
...
def change_username_account1(chat_id, session_id, csrf_token, random_username):
    ...
    try:
        ...
        errors_count += 1
        bot.send_message(chat_id, f"<b> - Error with KORN Swap Target Session: {str(e)}</b>", parse_mode='HTML')
...
def show_main_menu(chat_id):
    ...
    buttons = [
        ...
        "Swapper", "Settings", "Close Bot", "Developer"
    ]
...
def start(message):
    ...
    if chat_id not in PREMIUM_USERS:
        bot.reply_to(message, "<b> - Access restricted to premium subscribers.</b>", parse_mode='HTML')
        return
...
@bot.message_handler(func=lambda message: message.text in [
    ...
    "Run Main Swap", "BackUp Mode", "Threads Swap", "Webhook", "Bio", "Name", "Back", "Stop KORN", "Developer"
])
def handle_menu_navigation(message):
    ...
    if text == "Developer":
        bot.send_message(
            chat_id,
            f"<b> 🖥️ developer contact</b>\n\n"
            f"📸 Instagram https://instagram.com/activistmaazgotnochill\n"
            f"🔔 Telegram https://t.me/wm_h70/"
        )
        return
    if text == "Stop KORN":
        bot.send_message(chat_id, "<b> - KORN Stopped</b>", parse_mode='HTML')
        show_swapper_menu(chat_id)
        return
...
def save_main_session(message):
    chat_id = message.chat.id
    if chat_id not in PREMIUM_USERS:
        bot.send_message(chat_id, "<b> - Access restricted to premium subscribers.</b>", parse_mode='HTML')
        return
    init_session_data(chat_id)
